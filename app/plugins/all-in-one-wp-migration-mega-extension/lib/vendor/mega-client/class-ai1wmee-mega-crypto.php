...
class Ai1wmee_Mega_Crypto {

	public static function prepare_key( $str ) {
		$password_key = Ai1wmee_Mega_Utils::a32_to_str(
			array(
				0x93C467E3,
				0x7DB0C7A4,
				0xD1BE3F81,
		...
		for ( $r = 65536; $r--; ) {
			for ( $j = 0; $j < $total; $j += 4 ) {
				...
				$password_key = self::encrypt_aes_cbc( Ai1wmee_Mega_Utils::a32_to_str( $key ), $password_key );
			}
		}
	...
	public static function prepare_key_pwd( $password ) {
		return self::prepare_key( Ai1wmee_Mega_Utils::str_to_a32( $password ) );
	}

	public static function string_hash( $s, $aes_key ) {
		$s32 = Ai1wmee_Mega_Utils::str_to_a32( $s );
		$h32 = array( 0, 0, 0, 0 );
		...
		$h32 = Ai1wmee_Mega_Utils::a32_to_str( $h32 );

		for ( $i = 16384; $i--; ) {
			$h32 = self::encrypt_aes_cbc( $aes_key, $h32 );
		}

		$h32 = Ai1wmee_Mega_Utils::str_to_a32( $h32 );

		return Ai1wmee_Mega_Utils::a32_to_base64( array( $h32[0], $h32[2] ) );
	}
	...
	public static function encrypt_aes_cbc( $key, $data ) {
		if ( version_compare( PHP_VERSION, '5.4.0', '>=' ) && extension_loaded( 'openssl' ) && in_array( 'AES-128-CBC', array_map( 'strtoupper', @openssl_get_cipher_methods() ) ) ) {
			...
			$data = Ai1wmee_Mega_Utils::str_pad( $data );
	...
	public static function encrypt_aes_cbc_a32( $key, $data ) {
		return Ai1wmee_Mega_Utils::str_to_a32(
			Ai1wmee_Mega_Crypto::encrypt_aes_cbc(
				Ai1wmee_Mega_Utils::a32_to_str( $key ),
				Ai1wmee_Mega_Utils::a32_to_str( $data )
			)
		);
	}
	...
	public static function decrypt_aes_cbc_a32( $key, $a ) {
		return Ai1wmee_Mega_Utils::str_to_a32(
			Ai1wmee_Mega_Crypto::decrypt_aes_cbc(
				Ai1wmee_Mega_Utils::a32_to_str( $key ),
				Ai1wmee_Mega_Utils::a32_to_str( $a )
			)
		);
	}
	...
	public static function encrypt_node_attribute( $attr, $key ) {
		...
		return self::encrypt_aes_cbc( Ai1wmee_Mega_Utils::a32_to_str( $key ), $attr );
	}
	...
	public static function decrypt_node_attribute( $attr, $key ) {
		...
		$key = Ai1wmee_Mega_Utils::a32_to_str( $key );

		$attr = self::decrypt_aes_cbc( $key, $attr );
		$attr = Ai1wmee_Mega_Utils::str_depad( $attr );

		if ( substr( $attr, 0, 6 ) !== 'MEGA{"' ) {
			return false;
		}

		$attr = json_decode( Ai1wmee_Mega_Utils::from8( substr( $attr, 4 ) ), true );
